            ...
            try:
                if h.check():
                    if h.max_num_corrections is not None \
                        ...
                        if h.raise_on_max:
                            raise CustodianError(msg+" Skipping it.", True, h)
                        else:
                            logger.warning(msg)
                            continue
...
class ErrorHandler(MSONable):
    ...
    fixed maximum number of times. If the maximum number is reached the code
    ...
    as class attributess of the subclass or as customizable options setting
